class Outline extends PostProcessingEffect {
    ...
    apply(normalsTexture,objectIdTexture,depthTexture) {
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.fbo.fbo);
        gl.clear(gl.COLOR_BUFFER_BIT);
        this.program.use();
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, normalsTexture);
        gl.uniform1i(this.program.getUniformLocation(`normalTexture`), 0);
        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, objectIdTexture);
        gl.uniform1i(this.program.getUniformLocation(`objectIDTexture`), 1);
        ...
        gl.uniform1i(this.program.getUniformLocation(`contourMethod`), 1);
